...
class MotWanModel(torch.nn.Module):
    ...
    def compute_freqs_mot(self, f, h, w, end: int = 1024, theta: float = 10000.0):
        ...
        f_freqs_cis = precompute_freqs_cis(self.head_dim - 2 * (self.head_dim // 3), 0, end, theta)

        # ========== 修改前 ==========
        # 参考视频的空间高度和宽度起点是 0，与目标视频完全重合
        h_freqs_cis = precompute_freqs_cis(self.head_dim // 3, 0, end, theta)
        w_freqs_cis = precompute_freqs_cis(self.head_dim // 3, 0, end, theta)

        # ========== 修改后 ==========
        # 将参考视频在高度和宽度上都偏移到“左上角”的对角位置 (-h, -w)
        # h_freqs_cis = precompute_freqs_cis(self.head_dim // 3, -h, end, theta)
        # w_freqs_cis = precompute_freqs_cis(self.head_dim // 3, -w, end, theta)
